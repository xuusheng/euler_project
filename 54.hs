...
list=[["8C", "TS", "KC", "9H", "4S", "7D", "2S", "5D", "3S", "AC"],
...
flush c = (3 - (last c) + (head c)) `div` 3 * 26
kind x=10*(length $ filter (==0) y) + (length $ filter (==0) z) + (length $ filter (==1) y) `div` 4 * 25
  where y=zipWith (-) (tail x) (init x)
        z=zipWith (+) (tail y) (init y)
headPart x = sort $ map (translateHead . head) x
lastPart x = sort $ map (translateLast . last) x

vip x =  reverse $ sort $ map (\x->10^(length x)*(head x)) $group x
...
value x = [(kind (headPart x)) + (flush (lastPart x))] ++ (vip (headPart x))
...
euler54 x = compare'  (value (take 5 x)) (value (drop 5 x))

result = sum $ map euler54 list